...
public class BayesClassifier<T, K> extends Classifier<T, K> {
     ...
     */
    private SortedSet<ClassificationResult<T, K>> categoryProbabilities(
         ...
         * needed.
         */
        SortedSet<ClassificationResult<T, K>> probabilities =
                new TreeSet<ClassificationResult<T, K>>(
                        new Comparator<ClassificationResult<T, K>>() {

                    @Override
                    public int compare(ClassificationResult<T, K> o1,
                            ClassificationResult<T, K> o2) {
        ...
        for (K category : this.getCategories())
            probabilities.add(new ClassificationResult<T, K>(
                    features, category,
     ...
     */
    @Override
    public ClassificationResult<T, K> classify(Collection<T> features) {
        SortedSet<ClassificationResult<T, K>> probabilites =
     ...
     */
    public Collection<ClassificationResult<T, K>> classifyDetailed(
            Collection<T> features) {
